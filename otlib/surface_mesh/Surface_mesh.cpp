...
operator=(const Surface_mesh& rhs)
{
    if (this != &rhs)
    {
        ...
        vnormal_  = get_vertex_property<Normal>("v:normal");
        fnormal_  = get_face_property<Normal>("f:normal");
...
void
Surface_mesh::
get_neighbours2(Surface_mesh::Vertex v0, unsigned int depth, std::vector<Surface_mesh::Vertex> &n) const
{
  typedef std::pair<Surface_mesh::Vertex, unsigned int> VType;

  std::vector<bool> is_visited, is_added;
  is_visited.resize(vertices_size(), false);
  is_added.resize(vertices_size(), false);

  n.clear();

  std::queue<VType> q;
  q.push( VType(v0, 0) );
  is_added[v0.idx()] = true;

  while(!q.empty()){
    Surface_mesh::Vertex v = q.front().first;
    unsigned int d         = q.front().second;

    q.pop();

    if(is_visited[v.idx()] == false && d < depth){
      Surface_mesh::Vertex_around_vertex_circulator vv_cir = vertices(v);
      for(Surface_mesh::Vertex_around_vertex_circulator it=vv_cir.begin(); it!=vv_cir.end(); ++it){
        Surface_mesh::Vertex cv = to_vertex(it.halfedge());

        if(is_added[cv.idx()] == false){
          n.push_back(cv);
          q.push( VType( cv, d+1 ));

          is_added[cv.idx()] = true;
        }
      }
    }

    is_visited[v.idx()] = true;
  }
}

void
Surface_mesh::
get_neighbours(Surface_mesh::Vertex v0, double scale, std::vector<Surface_mesh::Vertex> &n) const
{
  const Point& p0 = position(v0);
  ...
  std::queue<Surface_mesh::Vertex> q;
  q.push( v0 );
  is_added[v0.idx()] = true;

  while(!q.empty()){
    Surface_mesh::Vertex v = q.front();
    q.pop();

    if(is_visited[v.idx()] == false){
      Surface_mesh::Vertex_around_vertex_circulator vv_cir = vertices(v);
      for(Surface_mesh::Vertex_around_vertex_circulator it=vv_cir.begin(); it!=vv_cir.end(); ++it){
        Surface_mesh::Vertex cv = to_vertex(it.halfedge());

        const Point& p = position(cv);
        double d = (p-p0).norm();

        if(is_added[cv.idx()] == false && d < scale){
          n.push_back(cv);
          q.push(cv);
...
void
Surface_mesh::
get_nring_faces(Surface_mesh::Face f0, unsigned int depth, std::vector<Surface_mesh::Face> &n) const
{
  typedef std::pair<Surface_mesh::Face, unsigned int> FType;

  std::vector<bool> is_visited, is_added;
  is_visited.resize(faces_size(), false);
  is_added.resize(faces_size(), false);

  n.clear();

  std::queue<FType> q;
  q.push( FType(f0, 0) );
  is_added[f0.idx()] = true;

  while(!q.empty()){
    Surface_mesh::Face f = q.front().first;
    unsigned int d       = q.front().second;

    q.pop();

    if(is_visited[f.idx()] == false && d < depth){
      Surface_mesh::Halfedge h, h0;
      h = h0 = halfedge(f);
      do{
        if(!is_boundary(edge(h))){
          Surface_mesh::Face cf = face(opposite_halfedge(h));
          if(is_added[cf.idx()] == false){
            n.push_back(cf);
            q.push( FType( cf, d+1 ));

            is_added[cf.idx()] = true;
          }
        }
    ...
    is_visited[f.idx()] = true;
  }
}

void
Surface_mesh::
get_nring_faces(Surface_mesh::Vertex v0, unsigned int depth, std::vector<Surface_mesh::Face> &n) const
{
  typedef std::pair<Surface_mesh::Vertex, unsigned int> VType;
  ...
  while(!q.empty()){
    ...
    if(is_visited[v.idx()] == false && d < depth){
      ...
      for(Surface_mesh::Vertex_around_vertex_circulator it=vv_cir.begin(); it!=vv_cir.end(); ++it){
        Surface_mesh::Vertex cv = to_vertex(it.halfedge());
        Surface_mesh::Face cf = face(it.halfedge());

        if(is_valid(cf) && is_added[cf.idx()] == false){
          n.push_back(cf);
          q.push( VType( cv, d+1 ));
...
void
Surface_mesh::
get_neighbouring_faces(Surface_mesh::Face f0, double scale, std::vector<Surface_mesh::Face> &n) const
{
  const Point& p01 = position(from_vertex(halfedge(f0)));
  const Point& p02 = position(to_vertex(halfedge(f0)));
  const Point& p03 = position(to_vertex(next_halfedge(halfedge(f0))));

  const Point& p0 = (p01+p02+p03)/3.;
  ...
  while(!q.empty()){
    Surface_mesh::Face f = q.front();
    q.pop();

    if(is_visited[f.idx()] == false){
      Surface_mesh::Halfedge h, h0;
      h = h0 = halfedge(f);
      do{
        if(!is_boundary(edge(h))){
          ...
          const Point& p1 = position(from_vertex(halfedge(cf)));
          const Point& p2 = position(to_vertex(halfedge(cf)));
          const Point& p3 = position(to_vertex(next_halfedge(halfedge(cf))));

          double d = std::min((p1-p0).norm(), std::min((p2-p0).norm(), (p3-p0).norm()));

          if(is_added[cf.idx()] == false && d <= scale){
            n.push_back(cf);
            q.push(cf);
...
triangulate(Face f)
{
	...
	Halfedge_property<unsigned int> vIndexes = halfedge_property<unsigned int>("vIndex");
    ...
    while (to_vertex(next_halfedge(next_h)) != start_v)
    {
        ...
        /* Update Halfedge */
        vIndexes[new_h] = vIndexes[next_next_h];
        vIndexes[opposite_halfedge(new_h)] = vIndexes[base_h];
...
compute_face_normal(Face f) const
{
    ...
    Vec3 p0 = make_vec3(vpoint_[to_vertex(h)]);
    h = next_halfedge(h);
    Vec3 p1 = make_vec3(vpoint_[to_vertex(h)]);
    h = next_halfedge(h);
    Vec3 p2 = make_vec3(vpoint_[to_vertex(h)]);
    ...
    else // face is a general polygon
    {
        ...
        do
        {
            ...
            p0 = p1;
            p1 = p2;
            p2 = make_vec3(vpoint_[to_vertex(h)]);
        }
        while (h != hend);
...
compute_vertex_normal(Vertex v) const
{
    ...
    if (h.is_valid())
    {
        ...
        do
        {
            if (!is_boundary(h))
            {
                p1 = make_vec3(vpoint_[to_vertex(h)]);
                p1 -= p0;
...
Surface_mesh::reversed_info
Surface_mesh::
collapse_with_reversed_info(Halfedge h)
{
    Halfedge o = opposite_halfedge(h);
    Vertex vh = to_vertex(h);
    Vertex vo = to_vertex(o);
    Vertex vhn =  to_vertex(next_halfedge(h));
    Vertex von =  to_vertex(next_halfedge(o));

    // store info
    reversed_info ri;

    // Face
    Face_around_vertex_circulator fc_it, fc_end;
    fc_it = fc_end = faces(vo);
    do
    {
        ri.f.push_back(std::pair<Face,Face_connectivity>(*fc_it,fconn_[*fc_it]));
    }
    while (++fc_it != fc_end);
    ...
    do
    {
        ri.he.push_back(std::pair<Halfedge,Halfedge_connectivity>(he,hconn_[he]));
        ri.he.push_back(std::pair<Halfedge,Halfedge_connectivity>(prev_halfedge(he),hconn_[prev_halfedge(he)]));
        ri.he.push_back(std::pair<Halfedge,Halfedge_connectivity>(next_halfedge(he),hconn_[next_halfedge(he)]));

        he = opposite_halfedge(next_halfedge(he));
    }
    while (he != he_end);
    ...
    //Vertex
    ri.v.push_back(std::pair<Vertex,Vertex_connectivity>(vh,vconn_[vh]));
    ri.v.push_back(std::pair<Vertex,Vertex_connectivity>(vo,vconn_[vo]));
    ri.v.push_back(std::pair<Vertex,Vertex_connectivity>(vhn,vconn_[vhn]));
    ri.v.push_back(std::pair<Vertex,Vertex_connectivity>(von,vconn_[von]));
...
void
Surface_mesh::
reverse_collapse(reversed_info ri)
{
    //restore vertex
    std::list<std::pair<Vertex,Vertex_connectivity> >::iterator vit;
    for(vit=ri.v.begin(); vit != ri.v.end(); ++vit) {
        vconn_[(*vit).first] = (*vit).second;
        if(vdeleted_[(*vit).first]) {
            vdeleted_[(*vit).first] = false; --deleted_vertices_;
        }
    }

    //restore halfedge
    std::list<std::pair<Halfedge,Halfedge_connectivity> >::iterator heit;
    for(heit=ri.he.begin(); heit != ri.he.end(); ++heit) {
        hconn_[(*heit).first] = (*heit).second;
        if(edeleted_[edge((*heit).first)]) {
            edeleted_[edge((*heit).first)] = false; --deleted_edges_;
        }
    }

    //restore face
    std::list<std::pair<Face,Face_connectivity> >::iterator fit;
    for(fit=ri.f.begin(); fit != ri.f.end(); ++fit) {
        fconn_[(*fit).first] = (*fit).second;
        if(fdeleted_[(*fit).first]) {
            fdeleted_[(*fit).first] = false; --deleted_faces_;
        }
    }
}
...
void Surface_mesh::diffuse_vert_normal(Vertex v, float invsigma2, unsigned int &flag_curr, std::vector<unsigned int> &flags)
{
    Vertex_property<Point> vnormal = get_vertex_property<Point>("v:normal");
    Vertex_property<Point> snormal = get_vertex_property<Point>("v:snormal");
    Vertex_around_vertex_circulator vv, vv_end;
    vv = vertices(v);
    vv_end = vv;
    Point n = vnormal[v];
    snormal[v] = n;
    ...
    while(!boundary.empty()) {
        ...
        w *= dotProd;
        snormal[v] += w*vnormal[vn];
        sum_w += w;
        vv = vertices(vn);
        vv_end = vv;
        do {
            if(flags[(*vv).idx()] == flag_curr)
                continue;
            boundary.push_back(*vv);
        }while(++vv != vv_end);
    }
...
void Surface_mesh::diffuse_normals(float sigma)
{
    std::vector<unsigned int> flags;
    flags.resize(n_vertices(),0);
    unsigned int flag_curr = 0;
    float invsigma2 = 1.f/ (sigma*sigma);
    Vertex_property<Point> snormal = add_vertex_property<Point>("v:snormal");
    Vertex_property<Point> vnormal = get_vertex_property<Point>("v:normal");
    for(Vertex_iterator v_it = vertices_begin(); v_it != vertices_end(); ++v_it) {
        diffuse_vert_normal(*v_it, invsigma2, flag_curr, flags);
        snormal[*v_it].normalize();
    }
    for(Vertex_iterator v_it = vertices_begin(); v_it != vertices_end(); ++v_it)
        vnormal[*v_it] = snormal[*v_it];
    remove_vertex_property<Point>(snormal);
}
