...
public: //------------------------------------------------------ reverse info types
    /// This type stores the information to reverse a collapse
    struct reversed_info
    {
        std::list<std::pair<Vertex,Vertex_connectivity> > v;
        std::list<std::pair<Halfedge,Halfedge_connectivity> > he;
        std::list<std::pair<Face,Face_connectivity> > f;
    };
...
public: //--------------------------------------------------- property handling
    ...
    Property_container& internal_vertex_properties() { return vprops_; }
...
public: //--------------------------------------------- neighbour computation

    //@{
    void get_neighbours2(Surface_mesh::Vertex v0, unsigned int depth, std::vector<Surface_mesh::Vertex>& n) const;
    void get_neighbours(Surface_mesh::Vertex v0, double       scale, std::vector<Surface_mesh::Vertex>& n) const;
    //@}

    //@{
    void get_nring_faces(Surface_mesh::Face   f0, unsigned int depth, std::vector<Surface_mesh::Face>& n) const;
    void get_nring_faces(Surface_mesh::Vertex v0, unsigned int depth, std::vector<Surface_mesh::Face>& n) const;

    void get_neighbouring_faces(Surface_mesh::Face f0, double scale, std::vector<Surface_mesh::Face>& n) const;
    //@}
...
public: //--------------------------------------------- higher-level operations
    ...
    /// \sa trianglate()
    std::vector<Surface_mesh::Face> triangulate(Face f);
    ...
    reversed_info collapse_with_reversed_info(Halfedge h);
    void reverse_collapse(reversed_info ri);
    ...
    void diffuse_vert_normal(Vertex v, float invsigma2, unsigned int &flag_curr, std::vector<unsigned int> &flags);

    void diffuse_normals(float sigma);
